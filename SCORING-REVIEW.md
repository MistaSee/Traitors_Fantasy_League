...
**Shrink teams based on the remaining eligible field, not just episode number.** The workbook’s 2+6 then 1+3 schedule is a sensible starting point. However, if fewer than two Traitors remain, the current requirement becomes impossible. Before opening each draft, choose quotas everyone can meet. A sensible policy is 2+6 while at least 3 Traitors and 9 Faithful remain, then 1+3 while at least 2 Traitors and 5 Faithful remain, then 1+2 where possible. If one role has no eligible characters, publish an emergency open-role rule before the draft opens; do not invent it after picks arrive. The current app supports configurable role counts, not open-role slots.
