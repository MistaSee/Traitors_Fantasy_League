...
import {checkEpisodeOne} from './browser-episode-one.mjs';
...
assert.equal(await page.locator('#kind').inputValue(),'preseason','Preseason must open by default before the season');
assert.equal(await page.locator('[data-pick]').count(),21,'All celebrities are available before roles are revealed');
await page.locator('#kind').selectOption('weekly');
assert.equal(await page.locator('#episode').inputValue(),'1','An open episode 1 is the first team draft');
await page.locator('#episode').selectOption('2');
await page.getByRole('button',{name:'Make preseason picks',exact:true}).click();
assert.equal(await page.locator('#kind').inputValue(),'preseason','Empty weekly draft provides a route back to preseason');
...
await page.reload();await page.getByRole('button',{name:'Review preseason picks',exact:true}).click();
...
await page.locator('#admin-episode').selectOption('2');await page.locator('summary').filter({hasText:'Active cast and roles'}).click();await page.getByRole('button',{name:'Copy starting roles'}).click();await page.getByRole('button',{name:'Save episode setup',exact:true}).click();
