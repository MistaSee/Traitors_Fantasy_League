...
let api, data, tab='Standings', episode=2, kind='weekly', selected=[], captain='', dirty=false;
...
async function refresh(){if(!demo)data=await rpc('read_league');dirty=false;render();}
async function saveState(next){
 ...
 await refresh();notice('League changes saved.');
}
function login(){
...
function render(){
 ...
 bind('#signout','click',async()=>{if(demo){if(!confirm('Clear this browser’s demo league?'))return;localStorage.removeItem('round-table-demo');location.reload();}else{await api.auth.signOut();data=null;$('#account').innerHTML='';login();}});
 ...
 document.querySelectorAll('[data-tab]').forEach(b=>b.onclick=()=>{if(dirty&&!confirm('Leave without saving these changes?'))return;dirty=false;tab=b.dataset.tab;render();});
...
function draft(){
 ...
 bind('#kind','change',()=>{kind=$('#kind').value;draft();});bind('#episode','change',()=>{episode=Number($('#episode').value);draft();});
 ...
 await refresh();notice('Your picks are saved.');});
}
function cast(){
...
function admin(){
 ...
 events();bind('#scored-character','change',events);
 bind('#counts','submit',async e=>{e.preventDefault();const next=structuredClone(s),id=$('#scored-character').value;const counts={};document.querySelectorAll('[data-count]').forEach(el=>counts[el.dataset.count]=Number(el.value));next.episodes[episode-1].counts[id]=counts;await saveState(next);});
 bind('#rules-form','submit',async e=>{e.preventDefault();const next=structuredClone(s);document.querySelectorAll('[data-points]').forEach(el=>next.rules.find(r=>r.id===el.dataset.points).points=Number(el.value));await saveState(next);});
